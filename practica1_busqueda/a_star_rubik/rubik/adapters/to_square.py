...
class ToSquare(CubeAdapter):
    def adapt(self,fill_with="",face_order=["U", "L", "F", "R", "B", "D"],
              prepend_face=False, wrap_with=None,value_map=None):
       pattern = []
       ...
       if value_map:
            sublists = [[value_map.get(color, color) for color in sublist] for sublist in sublists]
       if prepend_face:
